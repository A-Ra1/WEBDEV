...
public class BoardList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<tr class=danger>"); // danger : red, success : green
		...
		for(BoardVO vo:list) {
			...
			out.println("<td class=text-letfr width=45%>"
			+"<a href=BoardDetail?no="+vo.getNo()+">"
			+vo.getSubject()+"</a></td>");
			out.println("<td class=text-center width=15%>"+vo.getName()+"</td>");
			out.println("<td class=text-center width=20%>"+vo.getRegtDate().toString()+"</td>");
